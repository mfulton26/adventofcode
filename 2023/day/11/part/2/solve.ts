...
function expandUniverse(
  positions: ReturnType<typeof parseGalaxyPositions>,
  multiplier: number,
) {
    ...
    positions.sort(({ [coordinate]: a }, { [coordinate]: b }) => a - b).forEach(
      (position, i, positions) => {
        ...
        for (let j = i; j < positions.length; j++) {
          positions[j][coordinate] += space * (multiplier - 1);
        }
      },
    );
  }
...
export default function solve(input: string, { multiplier = 1000000 } = {}) {
  const galaxyPositions = parseGalaxyPositions(input);
  expandUniverse(galaxyPositions, multiplier);
  let sum = 0;
