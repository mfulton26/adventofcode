...
export async function solve({ part } = {}) {
  ...
  for (const part of parts) await solvePart(year, day, part, input);
}
...
export async function solvePart(year, day, part, input) {
  ...
  try {
    const { default: solve } = await getSolver(year, day, part);
    if (solve) {
 ...
 * @param {number} day
 * @param {number} part
 * @returns {Promise<{ default: (input: string) => unknown }>}
 */
export async function getSolver(year, day, part) {
  ...
  return await import(`https://bundle.deno.dev/${url}`);
}
