export default function solve(input: string, { moves = 100 } = {}) {
  ...
  return Array.from(valuesAfter(cups, 1)).join("");
}

function parseCups(text: string) {
...
function* valuesAfter(map: Map<number, number>, reference: number) {
  for (
